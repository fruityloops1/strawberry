...
void RichPresence::CurrentSongChanged(const Song &song) {
  SetTimestamp(0);
  activity_.lengthSecs = song.length_nanosec() / 1e9;
...
void RichPresence::SendPresenceUpdate() {
  ...
  const qint64 startTimestamp = activity_.startTimestamp - activity_.seekSecs;

  presence_data.startTimestamp = startTimestamp;
  presence_data.endTimestamp = startTimestamp + activity_.lengthSecs;

  Discord_UpdatePresence(&presence_data);
}

void RichPresence::SetTimestamp(const qint64 seekMicroseconds) {
  activity_.startTimestamp = time(nullptr);
  activity_.seekSecs = seekMicroseconds / 1e6;
}
